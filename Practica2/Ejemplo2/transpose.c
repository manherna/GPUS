...
void writeMatrix(float *in, int n){
	int i, j;
	for(j=0; j < n; j++) {
		for(i=0; i < n; i++) 
			printf("%f ", in[j*n+i]);
...
int main(int argc, char **argv)
{
	...
	cl_mem dInput; //Device input
	cl_mem dOutput; //device output
	...
	//darray1D       =   malloc(size);
	//darray1D_trans =   malloc(size);

	dInput = clCreateBuffer(context, CL_MEM_READ_ONLY, size, NULL, NULL);
	dOutput = clCreateBuffer(context, CL_MEM_WRITE_ONLY, size, NULL, NULL);

	 err = clEnqueueWriteBuffer(commands, dInput, CL_TRUE, 0, size, array1D, 0, NULL, NULL);
    	if (err != CL_SUCCESS)
    	{
	...
	if ( clSetKernelArg(kernel, 0, sizeof(cl_mem), &dOutput) ||
         clSetKernelArg(kernel, 1, sizeof(cl_mem), &dInput) ||
	 clSetKernelArg(kernel, 2, (BLOCK_SIZE*BLOCK_SIZE *sizeof(cl_mem)), NULL) ||
         clSetKernelArg(kernel, 3, sizeof(cl_uint), &n) != CL_SUCCESS)
	{
	...
	if (check(array1D_trans_GPU, array1D_trans, n*n)){
		printf("CPU:\n");
		writeMatrix(array1D_trans, n);
		printf("GPU:\n");
		writeMatrix(array1D_trans_GPU, n);
		printf("\n\nTranspose Host-Device differs!!\n");
		printf("Original:\n");
		writeMatrix(array1D, n);
