...
__global__ 
void imageGradientGPU (float *im, float * im_out, int h, int width) {
	...
	if(i > 2 && i < h-2 && j >2 && j < width-2)
	{
		...
		im_out[i*width+j] = sqrtf((Gx*Gx)+(Gy*Gy));
		
	}
	
	else if (i<h && j<width) im_out[i*width+j] = 0.0;
	...
	int height, int width)
{
	...
	cudaMemcpy(im_GPU, im_out_GPU, IMG_SIZE, cudaMemcpyDeviceToDevice);
	
	imageGradientGPU<<<numBlocks,threadsPerBlock>>>(im_GPU, im_out_GPU, height, width);
